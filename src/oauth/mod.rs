...
const TWITTER_ENCODING: &AsciiSet = &NON_ALPHANUMERIC
.remove(b'_')
.remove(b'-')
.remove(b'~')
.remove(b'.');
...
impl OAuthClient {
    pub fn new(app_key: String, app_secret: String, user_token: String, user_secret: String) -> Self {
        OAuthClient {
            app_key,
            app_secret,
            user_token,
    ...
    pub fn auth_request(&self, mut request: Request, mut external_params: Vec<Parameter>) -> Result<Request, Box<dyn std::error::Error>> {
        ...
        let oauth_timestamp = SystemTime::now()
            ...
            .as_secs()
            .to_string();
        parameters.push(Parameter::new("oauth_timestamp".to_string(), oauth_timestamp.clone()));

        let nonce = Self::create_nonce()?;
        parameters.push(Parameter::new("oauth_nonce".to_string(), nonce.clone()));

        let signature = percent_encode(self.create_signature(
            request.method(),
            request.url(),
            parameters).as_bytes(),
            TWITTER_ENCODING).to_string();
    ...
    fn create_nonce() -> Result<String, std::string::FromUtf8Error> {
        ...
        String::from_utf8(base64::bytes_to_base64(nonce.to_vec())
            .as_bytes()
            .iter()
    ...
    fn create_signature(&self, method: &Method, url: &Url, mut params: Vec<Parameter>) -> String {
        let digest = Sha1::new();
        let key = self.app_secret.clone()
            + "&"
            + self.user_secret.as_str();
    ...
    fn test_encoding_thing() {
        ...
        */
        ...
        let base_string = vec![
            ...
            Url::parse("https://api.twitter.com/1.1/statuses/update.json").unwrap().to_string(),
